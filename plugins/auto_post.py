...
def detect_episode_range(name):
    # ONLY detect if explicit episode pattern exists
    if not re.search(r"\b(E|EP|Episode)\b", name, re.I):
        return None

    m = re.search(
        r"(?:E|EP|Episode)\s*(\d{1,2})\s*(?:-|–|to)\s*(?:E|EP|Episode)?\s*(\d{1,2})",
        name,
        re.I
    )
    if m:
...
async def auto_post(client, message):
    ...
    is_series = bool(
        season is not None
        or re.search(r"\bS\d{1,2}\b|\bSeason\b|\bEpisode\b", fname, re.I)
    )

    ep_range = detect_episode_range(fname) if is_series else None

    title = clean_title(raw_title)
    if not title:
        return

    file_year = extract_year(fname)
