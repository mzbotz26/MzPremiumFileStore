...
async def tmdb_fetch(title, year=None, is_tv=False):
    try:
        ...
        async with aiohttp.ClientSession() as s:
            ...
            for d0 in r["results"]:
                y = (d0.get("release_date","") or d0.get("first_air_date",""))[:4]
                if year and y == year:
                    mid = d0["id"]
                    break
            else:
...
async def auto_post(client,message):
    ...
    file_year = extract_year_from_filename(media.file_name)

    if not title_year:
        title_year = file_year
    ...
    display_title = f"{title} ({title_year})" if title_year else title

    key = merge_key(title, title_year, season)
