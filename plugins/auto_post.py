...
async def tmdb_fetch(title, year=None, is_tv=False):
    try:
        ...
        async with aiohttp.ClientSession() as s:
            r = await (await s.get(f"https://api.themoviedb.org/3/search/{url}?api_key={TMDB_API_KEY}&query={quote_plus(title)}")).json()
...
"""
        ...
        imdb_url = f"https://www.imdb.com/title/{imdb_id}/" if imdb_id else f"https://google.com/search?q={quote_plus(display_title+' imdb')}"
        search_url = f"https://t.me/{client.username}?inlinequery={quote_plus(display_title)}"
