...
def detect_combined_episode_range(name):
    nums = re.findall(r"(?:E|EP|EPI|Episode)?\s*(\d{1,2})", name, re.I)
...
def detect_audio(name):
    return [
        x.capitalize()
        for x in ["hindi","english","telugu","tamil","malayalam","marathi","kannada","punjabi"]
        if x in name.lower()
    ]
...
async def auto_post(client, message):
    ...
    ep_range = detect_combined_episode_range(media.file_name)
