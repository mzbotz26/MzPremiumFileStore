...
def clean_title(raw):
    ...
    raw = re.sub(r"\b(onlymovies|onlymoviiies|mzmoviiez|mzmoviies|telegram|tme|movieshub|filmyzilla|south|uncut|mk)\b","",raw,flags=re.I)
    raw = re.sub(r"\b(480p|720p|1080p|2160p|4k)\b","",raw,flags=re.I)
    raw = re.sub(r"\b(x264|x265|hevc|hdrip|webdl|webrip|bluray|brrip|hdtc|hdts|cam|dd5|dd5\.1|aac|dts)\b","",raw,flags=re.I)
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async def search(lang):
                async with session.get(f"https://api.themoviedb.org/3/search/{url}?api_key={TMDB_API_KEY}&query={title}&language={lang}") as r:
                    return await r.json()
...
async def auto_post(client,message):
    ...
    if loop not in locks[merge_key]:
        locks[merge_key][loop]=asyncio.Lock()

    async with locks[merge_key][loop]:
