...
def merge_key(title,season=None):
...
def detect_audio(name):
    langs=[x.capitalize() for x in ["hindi","english","telugu","tamil","malayalam","marathi"] if x in name.lower()]
...
def detect_source(name):
    for s in ["bluray","brrip","webdl","webrip","hdrip","cam","hdts","hdtc"]:
        if s in name.lower(): return s.upper()
    return "WEB-DL"
...
async def tmdb_fetch(title,is_tv=False,season=None):
    try:
        ...
        async with aiohttp.ClientSession() as s:
            ...
            if not r["results"]: return None,None,None,None,None,None
            mid=r["results"][0]["id"]
...
async def auto_post(client,message):
    ...
    if not poster or not story:
        t=await tmdb_fetch(title,is_series,season)
    ...
    if not story:
        story=f"{title} is an entertaining title loved by audiences."

    key=merge_key(title,season)
...
"""
        ...
        imdb_btn=InlineKeyboardButton("🎬 IMDb",url=f"https://www.imdb.com/title/tt{imdb_id}/" if imdb_id else f"https://google.com/search?q={title}+imdb")
        search_btn=InlineKeyboardButton("🔍 Search",url=f"https://t.me/{client.username}?inlinequery={title}")
        ...
        msg=await client.send_photo(POST_CHANNEL,poster,caption=head+line+footer,parse_mode=ParseMode.HTML,reply_markup=kb) if poster else await client.send_message(POST_CHANNEL,head+line+footer,parse_mode=ParseMode.HTML,reply_markup=kb)
