...
"""
        ...
        imdb_btn = InlineKeyboardButton(
            "🎬 IMDb",
            url=f"https://www.imdb.com/title/tt{imdb_id}/" if imdb_id else f"https://google.com/search?q={display_title}+imdb"
        )

        search_btn = InlineKeyboardButton(
            "🔍 Search",
            url=f"https://t.me/{client.username}?inlinequery={display_title}"
        )
        ...
        msg = await client.send_photo(
            ...
            parse_mode=ParseMode.HTML,
            reply_markup=kb
        ) if poster else await client.send_message(
            POST_CHANNEL,
            head+line+footer,
            parse_mode=ParseMode.HTML,
            reply_markup=kb
